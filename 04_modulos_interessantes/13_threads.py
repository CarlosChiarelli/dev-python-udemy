...
# cria um bloqueador (código só continua depois que a thread acabar)
# também garante que terminou de executar
t1.join()
...
class Ingressos:
    ...
    def comprar(self, quantidade):
        ...
        :rtype: None
        """
        # Tranca o método (pega o recurso pra ele)
        self.lock.acquire()

        if self.estoque < quantidade:
            print('Não temos ingressos suficientes.')
            # Libera o método (libera o recurso)
            self.lock.release()
            return
...
if __name__ == '__main__':
    ...
    # se esse trecho não for added então a última linha é executada antes das threads
    executando = True
    while executando:
        executando = False
